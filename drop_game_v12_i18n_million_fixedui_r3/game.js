...
function setHUDVisible(v){
  hudLeft.style.display = v ? "block" : "none";
  hudBottom.style.display = v ? "block" : "none";
}
setHUDVisible(false); // 初始：在選單與結束畫面不顯示
...
function loop(ts){
  ...
  if(state==="playing"){
    ...
    if(timeLeft<0){
      ...
      setHUDVisible(false); // 結束畫面隱藏 HUD
    }
    hudTimeEl.textContent=`${t("time")}: ${Math.ceil(timeLeft)}`;
